...
class PlannerPropTest extends JUnitSuite {
  ...
  def plannerInvariantProp(): Unit = {
    Checkers.check(Prop.forAll(RetzDataGen.application(owner), jobs, offers(fid), Gen.posNum[Int]) {
      (application: Application, jobs: List[Job], offers: List[Offer], maxStock) => {
        var appJobs = jobs.map(job => new AppJobPair(Optional.of(application), job))
        var planner: Planner = new NaivePlanner();
        println(maxStock)
        var plan: Plan = planner.plan(offers.asJava, appJobs.asJava, maxStock);
        (plan.getToBeAccepted.size() + plan.getToDecline.size() + plan.getToStock.size() == offers.size) &&
          (maxStock >= plan.getToStock.size())         &&
          (plan.getToBeLaunched.size() + plan.getToCancel.size() == jobs.size)
      }
    })
  }
}